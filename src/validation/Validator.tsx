...
export const VALIDATOR_PHONENUMBER = (val: any) => ({ type: VALIDATOR_TYPE_PHONENUMBER, val: val });
...
export const validate = (value:any, validators:any) => {
  ...
  for (const validator of validators) {
    ...
    if (validator.type === VALIDATOR_TYPE_PHONENUMBER) {
        isValid = isValid && value.trim().length >= validator.val && value.trim().length <= validator.val;
      }
  }
  return isValid;
};
